...
def extract_text_from_image(image):
    ...
    headers = {
        "x-rapidapi-key": API_KEY,  # Use the securely loaded API key
    ...
    # Return only the extracted text from the JSON response
...
if uploaded_file:
    ...
    im2_reg = Image.open(uploaded_file)
    ...
    st.image(im2_reg_resized, caption='Resized Uploaded Image', use_column_width=True)
    
    # Allow the user to draw points on the image
    ...
    canvas_result = st_canvas(
        ...
        drawing_mode="point",
        key="canvas",
        display_toolbar=False,  # Hide toolbar to minimize size
    )
